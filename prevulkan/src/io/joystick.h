...
class Joystick{
    public:
        // joystick state callbacks

        Joystick(int _id);

        void update(); //joystick doesnt have a callback, so we need to update it manually
        float axesState(int axis);
        unsigned char buttonState(int button);

        int getAxesCount();
        int getButtonCount();
        static int getId(int _id);
        bool isPresent();
        const char* getName();
    ...
    private:
        ...
        int buttonCount;
        //We dont know how many buttons there are until runtime, so we use a pointer to an array of unsigned chars
