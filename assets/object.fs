#version 330 core

struct Material {
    vec3 ambient;
    vec3 diffuse;
    vec3 specular;
    float shininess;
};

struct Light {
    vec3 position;

    vec3 ambient;
    vec3 diffuse;
    vec3 specular;
};

out vec4 FragColor;
in vec3 FragPos;
in vec3 Normal;
in vec2 TexCoord;

uniform Material material;
uniform Light light;

uniform vec3 viewPos;
// uniform sampler2D texture1;
// uniform sampler2D texture2;
// uniform float mixValue;

void main(){
    // FragColor = mix(texture(texture1, TexCoord), texture(texture2, TexCoord), mixValue);

    // ambient - constant
    vec3 ambient = light.ambient * material.ambient;

    // diffuse - is more about the position of the object in relation to the light
    vec3 norm = normalize(Normal);
    vec3 lightDir = normalize(light.position - FragPos);
    float diff = max(dot(norm, lightDir), 0.0); //when light is perpendicular+ to the normal, dot product is 0
    vec3 diffuse = light.diffuse * (diff * material.diffuse);

    // specular (is more about the position of the camera in relation to the object, 
    //as how the light gets more concentrated as you get closer to a reflected ray)
    vec3 viewDir = normalize(viewPos - FragPos);
    vec3 reflectDir = reflect(-lightDir, norm); //the bounce of the light direction
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess*128); //128 is arbitrary
    vec3 specular = light.specular * (spec * material.specular);

    FragColor = vec4(vec3(ambient + diffuse + specular), 1.0);
}