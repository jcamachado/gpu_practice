...
namespace ud {
     ...
     */
    VkDeviceSize UDBuffer::getAlignment(VkDeviceSize instanceSize, VkDeviceSize minOffsetAlignment) {
    ...
    UDBuffer::~UDBuffer() {
        ...
        vkFreeMemory(udDevice.device(), memory, nullptr);
    }
    /**
     ...
     * @return VkResult of the buffer mapping call
     */
    VkResult UDBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
        assert(buffer && memory && "Called map on buffer before create");
        return vkMapMemory(udDevice.device(), memory, offset, size, 0, &mapped);
    }
    ...
    void UDBuffer::unmap() {
        if (mapped) {
            vkUnmapMemory(udDevice.device(), memory);
            mapped = nullptr;
        }
    }
     ...
     *
     */
    VkResult UDBuffer::flushIndex(int index) { return flush(alignmentSize, index * alignmentSize); }
