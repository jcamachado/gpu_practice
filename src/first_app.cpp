...
namespace ud {
    ...
    class GravityPhysicsSystem {
        public:
            ...
            void update(std::vector<UDGameObject>& objs, float dt, unsigned int substeps = 1) {
            ...
            glm::vec2 computeForce(UDGameObject& fromObj, UDGameObject& toObj) const {
        ...
        private:
            void stepSimulation(std::vector<UDGameObject>& physicsObjs, float dt) {
    ...
    class Vec2FieldSystem {
        public:
            void update(
                ...
                std::vector<UDGameObject>& physicsObjs,
                std::vector<UDGameObject>& vectorField) {
    ...
    std::unique_ptr<UDModel> createSquareModel(UDDevice& device, glm::vec2 offset) {
    ...
    std::unique_ptr<UDModel> createCircleModel(UDDevice& device, unsigned int numSides) {
        std::vector<UDModel::Vertex> uniqueVertices{};
...
void FirstApp::run() {
    // create some models
    std::shared_ptr<UDModel> squareModel = createSquareModel(
        udDevice,
        {.5f, .0f}
    );  // offset model by .5 so rotation occurs at edge rather than center of square
    std::shared_ptr<UDModel> circleModel = createCircleModel(udDevice, 64);

    // create physics objects
    std::vector<UDGameObject> physicsObjects{};
    auto red = UDGameObject::createGameObject();
    ...
    auto blue = UDGameObject::createGameObject();
    ...
    for (int j = 0; j < gridCount; j++) {
        auto vf = UDGameObject::createGameObject();
    ...
    if (auto commandBuffer = udRenderer.beginFrame()) {
        // update systems
